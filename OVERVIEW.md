...
**当前状态**: v0.2.0 已发布（584 测试全绿）— 核心引擎 + GitHub/Notion 双向同步 + 写回闭环 + MCP Server (18 tools)

**下一步（v0.3）**: 知识文档层（节点挂载 MD + 继承）+ 工作台（activate_workbench）+ 三省 Protocol（中书决策 + 门下经验审查 + 尚书工程评审+执行）+ 全文搜索
...
```
📁 FocalPoint — AI 认知操作系统 [active]
  ├─ 第一层：记忆引擎 ✅                  v0.2.0 发布 (PyPI + ClawHub)
  │   ├─ 节点管理 + 状态机 ✅
  │   ├─ Heartbeat + 风险感知 ✅
  │   ├─ Context Bundle ✅
  │   ├─ GitHub + Notion 双向同步 ✅
  │   └─ MCP Server 18 tools ✅
  │
  ├─ 第二层：知识层 + 工作台 [active]      ← v0.3 下一步
  │   ├─ 知识文档（overview/requirements/architecture + 可扩展）
  │   ├─ 工作台（activate_workbench）
  │   ├─ 三省 Protocol（决策→审查→执行）
  │   └─ 全文搜索
  │
  └─ 第三层：协作层 [inbox]               ← v0.4+ 未来
      ├─ 多 Agent 任务分配
      └─ 跨 Agent 共享记忆
```
...
*读完这个文件，你应该知道：我们在建什么（FocalPoint — AI 认知操作系统）、建到哪了（记忆引擎完成，知识层+工作台下一步）、下一步做什么（v0.3 Work Mode）。*
