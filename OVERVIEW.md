...
FocalPoint 是 AI 的认知操作系统。记忆 + 注意力管理 + 工作流编排，让 AI agent 像一个有经验的团队一样工作。
...
| 版本 | 形态 | 状态 |
|------|------|------|
| v0.1-v0.2 | 记忆引擎（节点+状态机+Heartbeat+GitHub/Notion） | ✅ 已完成 |
| v0.3 | 知识层 + 工作台 + 三省 Protocol + 全文搜索 | ✅ 已完成 |
| v0.4.0 | Analytics Dashboard + 健康评分 + 节点浏览器 | ✅ 已完成 |
| v0.5 | Interview 流程 + AI 自动记录 | **← 下一步** |
| v0.6+ | 多 Agent 协作 | 未来 |
...
**当前状态**: v0.4.0 已发布（690 测试全绿）— 核心引擎 + GitHub/Notion 双向同步 + 写回闭环 + 知识文档层 + 工作台 + 三省 Protocol + 全文搜索 + Analytics Dashboard + MCP Server (24 tools)

**下一步（v0.5）**: Interview 流程 + AI 自动记录 — 提升三省输入质量，减少用户手动操作
...
**当前状态**: 架构设计完成。FPMS 覆盖 Layer 2（任务状态+叙事）和 Layer 4 部分（knowledge 知识文档）。CTO Agent 将成为 Layer 4 的第一个完整实例
...
```
📁 FocalPoint — AI 认知操作系统 [active]    v0.4.0 (PyPI + ClawHub)
  ├─ 第一层：记忆引擎 ✅
  │   ├─ 节点管理 + 状态机 ✅
  │   ├─ Heartbeat + 风险感知 ✅
  │   ├─ Context Bundle（角色过滤 + token 预算）✅
  │   └─ GitHub + Notion 双向同步 ✅
  │
  ├─ 第二层：知识层 + 工作台 ✅
  │   ├─ 知识文档（CRUD + 继承 + FTS 索引）✅
  │   ├─ 工作台（activate_workbench + 角色 prompt）✅
  │   ├─ 三省 Protocol（并行审查 + 打回升级）✅
  │   ├─ 全文搜索（FTS5 + CJK fallback）✅
  │   ├─ Analytics Dashboard（健康评分 + HTML 面板）✅
  │   └─ MCP Server 24 tools ✅
  │
  ├─ 第二层+：工作流增强 [inbox]           ← v0.5 下一步
  │   ├─ Interview 流程（参考 gstack）
  │   ├─ AI 自动记录（参考 OpenViking）
  │   └─ Token efficiency 优化
  │
  └─ 第三层：协作层 [inbox]
      ├─ 多 Agent 任务分配
...
| 白皮书 V3 | `1-vision/WhitePaper.md` | 愿景 + 七模块 + 文明等级 |
| 产品方向 ADR | `1-vision/ADR-product-direction.md` | 认知操作系统定位决策 |
| Work Mode PRD | `2-requirements/PRD-work-mode.md` | v0.3 完整需求 |
| Memory Architecture | `docs/FounderOS-Memory-Architecture-V1.md` | 六层记忆模型 |
| CTO Agent PRD | `docs/CTO-AGENT-PRD-V2.md` | 第一个 Office 的完整规格 |
| v0.3 验收 | `4-implementation/v03-acceptance.md` | Work Mode 验收清单 |
| Roadmap | `4-implementation/ROADMAP.md` | 全阶段交付计划 |
| 产品介绍 | `docs/marketing/PRODUCT-INTRO.md` | 竞品分析 + 产品定位 |
| 使用指南 | `docs/marketing/USAGE-GUIDE.md` | 安装 + 功能 + 模板 |
| Work Mode 指南 | `docs/WORK-MODE-GUIDE.md` | 工作台 + 三省使用方法 |

---

*读完这个文件，你应该知道：我们在建什么（FocalPoint — AI 认知操作系统）、建到哪了（v0.4.0 — 记忆引擎+知识层+工作台+三省+Analytics 已完成，690 tests）、下一步做什么（v0.5 Interview + 自动记录）。*
