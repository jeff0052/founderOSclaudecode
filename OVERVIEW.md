...
| v0.3.1 | 知识层 + 工作台 + 三省 Protocol + 全文搜索 | ✅ 已完成 |
| v0.4+ | 多 Agent 协作 | **← 下一步** |
...
**当前状态**: v0.3.4 已发布（667 测试全绿）— 核心引擎 + GitHub/Notion 双向同步 + 写回闭环 + 知识文档层 + 工作台 + 三省 Protocol + 全文搜索 + MCP Server (23 tools)

**下一步（v0.4）**: 多 Agent 协作 — 任务分配、跨 Agent 共享记忆、并发控制
...
```
📁 FocalPoint — AI 认知操作系统 [active]    v0.3.1 (PyPI + ClawHub)
  ├─ 第一层：记忆引擎 ✅
  │   ├─ 节点管理 + 状态机 ✅
  │   ├─ Heartbeat + 风险感知 ✅
...
*读完这个文件，你应该知道：我们在建什么（FocalPoint — AI 认知操作系统）、建到哪了（v0.3.4 — 记忆引擎+知识层+工作台+三省 已完成，667 tests）、下一步做什么（v0.4 多 Agent 协作）。*
